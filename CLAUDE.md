...
- **Founder video** — Alexandra's YouTube Short (`JejpUNvq6Co`, vertical) appears in the founder block on **both** the home page and `/about` (intentional duplication). It uses a lightweight `.video-facade` `<button>` (YouTube `hqdefault.jpg` poster + a `.vf-play` button) inside a portrait `.pic` frame; `site.js` swaps it for a `youtube-nocookie.com` autoplay iframe (`.vf-iframe`) on click. The caption ph-tag reads "Watch · Alexandra on MobileREMS". If the auto thumbnail ever letterboxes, the documented fallback is to point the poster `<img>` at `/assets/img/founder-hero.jpg`.
