...
- **`/partners` is the "For Providers" page** — provider-focused. The hero is a **two-column split** (`.provider-hero`): For-Providers copy on the left, the Ivorey "Partner With Us" form on the right. The rest of the page covers the four readings, what it does for the practice, patient groups, how it works, and a bottom "Request a partnership" section. **Geography on this page only is "South Puget Sound and Greater Seattle"**; every other page keeps "Western Washington" / "Gig Harbor". **The same "Partner With Us" form appears twice** (hero + bottom `#partner-form`) — the hero iframe uses id suffix `-hero` (`inline-qJDn7ir2vDwOw6re8NFz-hero`, matching `data-layout-iframe-id`) so its DOM id stays distinct from the bottom one; duplicate ids would break `form_embed.js` auto-resize. The hero CTA anchors to `#partner-form-hero`.
