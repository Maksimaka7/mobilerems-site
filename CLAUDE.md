...
- **Founder video** — Alexandra's YouTube Short (`JejpUNvq6Co`, vertical) plays via a lightweight `.video-facade` `<button>` (YouTube `hqdefault.jpg` poster + a `.vf-play` button); `site.js` swaps it for a `youtube-nocookie.com` autoplay iframe (`.vf-iframe`) on click. It appears in: the home **block 03 "How REMS works"** `.teaser .media` (caption "Watch · How REMS works"; the body's "View the brochure" button still opens `/assets/clinical-brief.pdf`), and the `/about` B2C column (caption "Watch · Alexandra on MobileREMS"). Block 03 reuses the founder Short id for now — swap the `data-yt` if a dedicated explainer is produced. If a thumbnail ever letterboxes, the fallback is to point the poster `<img>` at a local image. The home **block 06 "Why MobileREMS"** no longer holds the video — it shows the founder photo `assets/img/founder-probe.jpg` (Alexandra with the REMS probe, 853×1066) as `.founder .pic .founder-photo` (caption "Founder · Alexandra Kusherets · Certified REMS® Operator"). The old `.brochure-cover` (cover-image-links-to-PDF) component is gone; `assets/img/brochure-cover.jpg` is now unused.

- **`/about` is a B2C + B2B page** — order: page-hero → `.about-split` → "Who's behind the scan" → optional coaching → `.brandblock` → final CTA. The `.about-split` is two columns: **left (B2C)** = the founder video facade + personal story + a Book a Scan button; **right (B2B)** = an `.about-b2b` sticky card pitching the provider offer (Partner With Us + Clinical Brief). Then a `.aud-grid` 2×2 "Who's behind the scan" (provider-facing bio) and an optional coaching section. Stacks to one column at ≤980px. **Copy-accuracy flag:** Alexandra's bio claims here (work with Allergan Aesthetics, "twenty years", "a decade in nutrition", certified Echolight REMS® operator, certified health coach) and "Longevia" vs "Longevia Center" must be owner-verified before/with the physician outreach (same spirit as the provider-stats FTC note). The vendor spelling is **Echolight REMS®** (the source doc's "Ecolight" was a typo).
